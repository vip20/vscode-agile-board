...
import { BoardItem, Task } from './BoardItem';
...
import { BiArrowToLeft, BiArrowToRight, BiAddToQueue, BiTrash } from 'react-icons/bi';
import { COLUMN_ADD } from '../../src/core/constants';
...
function getBackgroundColor(isDraggingOver: boolean) {
  return isDraggingOver ? 'var(--vscode-editorGroup-dropBackground)' : 'transparent';
}

// Create and export the BoardColumn component
export const BoardColumn: React.FC<BoardColumnProps> = React.memo((props: BoardColumnProps) => {
  ...
  useEffect(() => {
    const filteredDir: string[] = props.columnNames.filter((x: string) => x !== props.column.title);
    if (filteredDir.indexOf(columnName) !== -1) {
      setNameErrMsg(`Column by name "${columnName}" already exists.`);
    } else {
      setNameErrMsg('');
    }
  }, [columnName, props.column.title, props.columnNames]);
  return (
    <Draggable draggableId={props.column.id} key={props.column.id} index={props.columnIndex}>
      {provided => {
          ...
          primary: {
            main: [
              ...
              },
              {
                ...
                isDisabled: props.column.isDefault || props.column.tasksIds.length > 0,
                leftIcon: <BiTrash />,
              },
            ],
          },
        ...
        return (
          <BoardColumnWrapper {...provided.draggableProps} ref={provided.innerRef}>
            <BoardColumnTitle {...provided.dragHandleProps}>
              ...
              >
                <ColumnTitle>
                  <span className="count">({props.column.tasksIds.length})</span>
                  &thinsp;
                  <InputBox
              ...
              <div ref={menuRef}>
                <span onClick={() => setMenuOpen(!menuOpen)} style={{ cursor: 'pointer' }}>
...
const TaskList = React.memo((props: BoardColumnProps) => {
  ...
  }, [isAdd, props.column.tasksIds.length]);
