...
export function getPriorityColorScheme() {
  const config = workspace.getConfiguration('vsagile');
  const priorityColorScheme = resolveHome(config.get('priorityColorScheme') || '');
...
export async function showQuickPick<T extends QuickPickItem, P extends QuickPickParameters<T>>({
  title,
  items,
  placeholder,
  activeItem,
...
export const reFetchConfig = (boardName: string, panel: WebviewPanel) => {
